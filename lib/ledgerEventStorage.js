...
class LedgerEventStorage {
  constructor(options) {
    ...
    // assign the function used for hashing events
    this.hashEvent = options.eventHasher;
  }
  ...
  add(event, meta, options, callback) {
    ...
    async.auto({
      hash: callback => {
        // hash the event
        this.hashEvent(event, callback);
      },
      insert: ['hash', (results, callback) => {
        // FIXME: Move named hash to hash function, do not assume ni:///
        nihash = 'ni:///sha-256;' + b64url(results.hash);
        // insert the block
        const now = Date.now();
        const record = {
          eventHash: database.hash(nihash),
          event: event,
        ...
        logger.debug('adding event', event.id);
      ...
      if(err) {
        if(database.isDuplicateError(err)) {
          return callback(new BedrockError(
            ...
            'DuplicateEvent', {eventHash: nihash}));
        }
        return callback(err);
      }
