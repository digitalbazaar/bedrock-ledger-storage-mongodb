...
module.exports = class LedgerBlockStorage {
  ...
  getLatestSummary(options, callback) {
    ...
    const projection = {'block.event': 0};
    const sort = {'meta.consensusDate': -1};
  ...
  _expandEvents(block, callback) {
    ...
    this.eventCollection.find(query).toArray((err, result) => {
      ...
      if(result.length !== hashes.length) {
        return callback(new BedrockError(
          'Unable to dereference one or more events.',
          'NotFoundError', {block}));
      }
      for(let i = 0; i < result.length; ++i) {
        events[eventsToFetch[result[i].meta.eventHash]] = result[i].event;
      }
      // FIXME: since block is mutated, does it even need to be returned?
      callback(null, block);
    });
  }
};
