...
class LedgerBlockStorage {
  ...
  getLatest(options, callback) {
    async.auto({
      getLatestConfigBlock: callback => {
        // find the latest config block with consensus
        const query = {
          'block.type': 'WebLedgerConfigurationBlock',
          'meta.deleted': {
            $exists: false
          },
          'meta.consensus': {
        ...
        this.collection.find(query).sort(
          {'meta.consensus': -1}).limit(1).toArray(callback);
      },
      ...
      const configBlock = results.getLatestConfigBlock[0].block;
      let eventBlock = configBlock;

      if(results.getLatestEventBlock) {
        eventBlock = results.getLatestEventBlock[0].block;
      }
      callback(null, {configurationBlock: configBlock, eventBlock: eventBlock});
    });
  }
