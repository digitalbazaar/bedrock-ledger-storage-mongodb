...
module.exports = class LedgerBlockStorage {
  ...
  get(blockId, options, callback) {
    ...
    async.auto({
      ...
      expandEvents: ['find', (results, callback) => {
        ...
        if(!results.find.block.event) {
          // no events to expand
          return callback(null, results.find.block);
        }
        for(const event of results.find.block.event) {
        ...
        if(Object.keys(eventsToFetch).length === 0) {
          // no event hashes to fetch, continue
          return callback(null, results.find.block);
        }
        // get all event hashes from event collection
        const query = {
          'meta.eventHash': {
            $in: Object.keys(eventsToFetch)
          },
          'meta.deleted': {
            $exists: false
          },
        ...
        this.eventCollection.find(query).each((err, result) => {
          if(err) {
          ...
          if(result === null) {
            // no more documents, expand all event hashes
            const allEvents = results.find.block.event;
            for(let i = 0; i <= allEvents.length; i++) {
              const isEventHash = typeof(allEvents[i]) === 'string' &&
                allEvents[i].startsWith('ni:///');
              if(isEventHash) {
                allEvents[i] = eventsToFetch[allEvents[i]];
              }
            }
            callback(null, results.find.block);
            return false;
          }
          // cache expanded event hash
          eventsToFetch[result.meta.eventHash] = result.event;
        });
      }]
    }, (err, results) => {
      if(err) {
      ...
      callback(null, {block: results.expandEvents, meta: results.find.meta});
    });
  }
   ...
   *
   * blockHeight - the height of the block that has consensus.
   * options - a set of options used when retrieving the block(s).
   * callback(err, block) - the callback to call when finished.
   *   err - An Error if an error occurred, null otherwise.
    ...
    async.auto({
      ...
      if(!results.find) {
        return callback(new BedrockError(
          ...
          'NotFoundError', {blockHeight: blockHeight}));
      }
  ...
  getLatest(options, callback) {
    ...
    async.auto({
      getLatestEventBlock: callback => {
        ...
        // chain
        // FIXME: can query for max `blockHeight` if blockchain supports it
        // as well (can we enforce that all support it?)
    ...
    async.auto({
      ...
      ensureUpdate: ['update', (results, callback) => {
        if(results.update.matchedCount !== 1) {
          return callback(new BedrockError(
            'Delete of block failed.', 'NotFoundError', {blockHash: blockHash}
          ));
        }
        callback();
      }]
