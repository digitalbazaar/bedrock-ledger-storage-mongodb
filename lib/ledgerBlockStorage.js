...
module.exports = class LedgerBlockStorage {
  ...
  getGenesis(options, callback) {
    ...
    this.collection.findOne(query, {block: 1, meta: 1}, (err, record) => {
      ...
      this._expandEvents(record.block, (err, block) => {
        if(err) {
          return callback(err);
        }
        callback(null, {
  ...
  getLatest(options, callback) {
    ...
    async.auto({
      ...
      expandEvents: ['getLatestEventBlock', (results, callback) => {
        if(results.getLatestEventBlock.length === 0) {
          return callback(new BedrockError(
            'No latest event block exists.',
            'NotFoundError'));
        }
        this._expandEvents(results.getLatestEventBlock[0].block, callback);
      }]
    }, (err, results) => {
      if(err) {
        return callback(err);
    ...
    this.eventCollection.find(query).each((err, result) => {
      ...
      if(result === null) {
        ...
        for(let i = 0; i <= events.length; i++) {
          const isEventHash = typeof(events[i]) === 'string' &&
            events[i].startsWith('ni:///');
          if(isEventHash) {
            events[i] = eventsToFetch[events[i]];
          }
        }
        callback(null, block);
        return false;
