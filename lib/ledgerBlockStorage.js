  ...
  get(blockId, options, callback) {
    ...
    async.auto({
      ...
      expandEvents: ['find', (results, callback) => {
        ...
        for(let event of results.find.block.event) {
          // TODO: Determine if we want to expand other hash types
          let isEventHash = typeof(event) === 'string' &&
            event.startsWith('ni:///');
          if(isEventHash) {
            eventsToFetch[event] = {};
          }
        ...
        this.eventCollection.find(query).each(function(err, result) {
          if(err) {
          ...
          if(result === null) {
            ...
            let allEvents = results.find.block.event;
            for(let i = 0; i <= allEvents.length; i++) {
              let isEventHash = typeof(allEvents[i]) === 'string' &&
    ...
    async.auto({
      ...
      if(!results.find) {
        return callback(new BedrockError(
          ...
          'NotFound', {blockHeight: blockHeight}));
      }
