...
class LedgerOperationStorage {
  constructor({eventCollection, ledgerNodeId, operationCollection}) {
    ...
    this.eventCollectionName = eventCollection.collectionName;
   ...
   * @param {string} [recordId] the recordId associated with the operation.
   ...
   *   `false` if not.
   */
  async exists({eventHash, recordId, explain = false, operationHash} = {}) {
    if(recordId) {
      assert.string(recordId, 'recordId');
      const query = {recordId: database.hash(recordId)};
      const options = {limit: 1, projection: {_id: 0, recordId: 1}};
      if(explain) {
