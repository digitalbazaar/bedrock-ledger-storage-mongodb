...
class LedgerEventStorage {
  ...
  /**
   * Filter a list of eventHashes.
   *
   * @param eventHash - a string or array of hashes to filter.
   * @param blockHeight - filter on `meta.blockHeight`.
   * @param consensus - filter on `meta.consensus`.
   *
   * @return a Promise that resolves to the hashes that satisfy the filter.
   */
  ...
  async getActiveConfig({blockHeight}) {
    assert.optionalNumber(blockHeight, 'blockHeight');
    // audit:storage-mongodb/ef205122-464b-409a-bffb-ef3dc42c2dac.md
    const query = {
    ...
    const records = await this.collection.find(query, projection).sort({
      'meta.blockHeight': -1,
      'meta.blockOrder': -1
    }).limit(1).toArray();
    if(records.length === 0) {
  ...
  async getLatestConfig() {
    ...
    // audit:storage-mongodb/780c89b8-e72e-4178-817a-26898d290d45.md
    const query = {
      'event.type': 'WebLedgerConfigurationEvent',
      'meta.consensus': true
    };
