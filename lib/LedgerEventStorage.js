...
const VALID_IDEMPOTENT_OPERATIONS = new Set(['set', 'unset']);
...
class LedgerEventStorage {
  ...
  async add({event, meta}) {
    ...
    if(!operationHash && _util.hasValue(
    ...
    try {
      const {event, meta} = (await this.collection.insertOne(
  ...
  /**
   * Takes an array of events and chunks them so they can be written
   * in bulk to mongodb. The function also resumes on duplicate error.
   *
   * @param {object} options - Options to use.
   * @property {Array<object>} options.events - An array of ledger events.
   *
   * @throws {Error} - Ignores DuplicateErrors, throws the rest.
   *
   * @returns {Promise<object>} Returns an object with duplicate hash ids.
  */
    ...
    for(let chunk of chunks) {
      ...
      while(chunk.length > 0) {
        try {
          ...
          if(!database.isDuplicateError(e) || !e.writeErrors) {
            throw e;
          }
          // if there are writeErrors then correct for them
          for(const bulkError of e.writeErrors) {
            dupHashes.push(chunk[bulkError.err.index].meta.eventHash);
            // remove events up to the dup and retry
            chunk = chunk.slice(bulkError.err.index + 1);
          }
        }
      }
    }
  ...
  getMany({blockHeight, eventHashes}) {
    const operationCollectionName =
      this.operationStorage.collection.collectionName;
    const lookupStage = {
      $lookup: {
  ...
  async getEffectiveConfig({blockHeight, explain = false}) {
    assert.number(blockHeight, 'blockHeight');
    const query = {
      // NOTE: the effective config does not include any configs that may be in
    ...
    if(records.length === 0) {
      throw new BedrockError(
        'An effective ledger configuration was not found.',
  ...
  async update({eventHash, patch}) {
    const id = nanoid();
    const {update} = _createUpdatePatch({patch, idempotent: false, id});
   ...
   * @param patch - a list of patch commands for the event
   */
  async updateMany({events}) {
    const id = nanoid();
    const eventUpdates = events.map(({eventHash, patch}) => {
      const {update} = _createUpdatePatch({patch, idempotent: true, id});
      return {
        updateOne: {
          filter: {'meta.eventHash': eventHash},
          update
        }
      };
    });

    const chunks = _util.chunkDocuments(eventUpdates);

    for(const chunk of chunks) {
      const {result} = await this.collection.bulkWrite(chunk, {ordered: false});
      if(result.nModified !== chunk.length) {
        throw new BedrockError(
          'Could not update event(s).', 'OperationError',
...
function _createUpdatePatch({patch, idempotent, id}) {
  ...
  for(const operation of patch) {
    if(idempotent && !VALID_IDEMPOTENT_OPERATIONS.has(operation.op)) {
      throw new BedrockError(
        'Only idempotent operations are allowed.',
        'NotAllowedError',
        {operation, validOperations: Array.from(VALID_IDEMPOTENT_OPERATIONS)}
      );
    }
    ...
    if(operation.op === 'set') {
      _.merge(setObject, operation.changes);
    } else if(operation.op === 'unset') {
      _.merge(unsetObject, operation.changes);
  ...
  // add update id
  update.$set['meta.uid'] = Buffer.from(id);
