...
class LedgerEventStorage {
  ...
  */
  ...
  async update({eventHash, patch}) {
    const {update} = _createUpdatePatch({patch, idempotent: false});
  ...
  async updateMany({events}) {
    const eventUpdates = events.map(({eventHash, patch}) => {
      const {update} = _createUpdatePatch({patch, idempotent: true});
      return {
        updateOne: {
...
function _createUpdatePatch({patch, idempotent}) {
