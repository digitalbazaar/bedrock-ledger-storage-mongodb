...
class LedgerEventStorage {
  ...
  */
  ...
  async update({eventHash, patch}) {
    const {update} = _createUpdatePatch({patch});
    ...
    for(const chunk of chunks) {
      try {
        ...
        if(result.writeErrors.length > 0) {
          writeErrors.push(...result.writeErrors);
        }
        if(result.writeConcernErrors.length > 0) {
          writeConcernErrors.push(...result.writeConcernErrors);
        }
      }
    }

    const hasError = errors.length > 0 || writeErrors.length > 0 ||
      writeConcernErrors.length > 0;
    if(hasError) {
      throw new BedrockError(
        'Could not update event(s).', 'OperationError',
        {errors, writeErrors, writeConcernErrors});
    }
  }
...
function _createUpdatePatch({patch}) {
