    ...
    eventCollection.aggregate([
      ...
      {$lookup: {
        ...
        pipeline: [
          ...
          {$project: {_id: 0, 'meta.operationHash': 1}},
          {$group: {_id: null, operationHashes: {
            $addToSet: '$meta.operationHash'
          }}},
          {$project: {_id: 0}},
        ],
        as: 'operations',
      }},
      {$project: {operationHashes: {$arrayElemAt: ['$operations', 0]}}},
      {$replaceRoot: {newRoot: '$operationHashes'}}
      // {$unwind: '$operations'}
    ]).toArray((err, result) => {
      if(err) {
        return callback(err);
