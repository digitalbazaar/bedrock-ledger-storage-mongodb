...
/**
 * Create a new ledger given an initial configuration event,
 * config event metadata, and a set of options.
 *
 ...
 *     the ledger.
 */
// FIXME: do we still need meta passed in here since configBlock is not written?
api.add = (configEvent, meta, options, callback) => {
  ...
  async.auto({
    ...
    insert: ['generateUuid', (results, callback) => {
      ...
      logger.debug('adding ledger', configEvent.input[0].ledger);
