...
api.add = (meta = {}, options = {}, callback) => {
  ...
  async.auto({
    ...
    services: callback => async.each(services, (service, callback) => api.use(
      service, (err, result) => {
        if(err) {
          return callback(err);
        }
    ...
    extendIndexes: ['createCollectionIndexes', (results, callback) => {
      ...
      async.eachSeries(services, (service, callback) => {
        async.auto({
          service: callback => injector.use(service, callback),
    ...
    extendLedgerStorage: ['ledgerStorage', (results, callback) => {
      ...
      async.eachSeries(services, (service, callback) => {
        async.auto({
          ...
          extend: ['service', (results, callback) => {
            ...
            Object.keys(service.api.storage).forEach(type => {
    ...
    callback(err, results.extendLedgerStorage);
  });
};
